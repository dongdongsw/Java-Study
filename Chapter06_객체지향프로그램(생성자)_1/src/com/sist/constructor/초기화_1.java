 ...
 *			생성자 => 서버 연결 : 필요시에만 사용
 *			-----------------
 *			메소드 => 구현 => 수정 / 추가 => 상속 = 필요시에만 사용
 *			-----------------
 * 			}
 * 
 * 		데이터형 클래스 :변수만 모아서 저장
 * 		액션 클래스 : 기능 => 메소드
 * 		혼합형 : 변수 + 메소드 => 브라우저 전송 : 조립
 * 
 * 
 * 
 * 
 ...
 * 			------------------
 * 			변수 
 * 				ㄴ 데이터를 따로 저장 => new 를 이용해 메모리 공간을 만든다
 * 									데이터를 저장 
 * 									인스턴스 변수
 * 									new를 이용해서 메모리가 따라 생성된다
 * 									new를 사용할 때는 반드시 생성자를 동반한다
 * 									ㄴ 형식
 * 										ㄴ new 생성자()
 * 										  -----------
 * 											new => 클래스 크기 확인
 ...
 * 													메모리 주소를 리턴
 * 											생성자 => 메모리에 값을 대입
 * 											리턴형 => int malloc(클래스) => new 연산자로 변경
